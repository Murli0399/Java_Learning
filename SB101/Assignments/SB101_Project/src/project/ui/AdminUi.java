...
public class AdminUi {
	...
	static void createAuction(Scanner sc) {
		...
		int quantity = -1;
		try {
			quantity = dao.checkQuantity(Integer.parseInt(id));
			if (quantity < 1) {
				System.out.println(ConsoleColors.RED + "		Unfortunately, the following item out of stock."
						+ ConsoleColors.RESET);
			} else {
				...
				try {
					dao.createAuction(Integer.parseInt(id), date, st, duration);
	...
	static void viewSoldProduct(Scanner sc) {
		...
		try {
			...
			if (x.equals("1")) {
				...
				String ch = sc.next();
				dao.viewSoldProductByCategory(Integer.parseInt(ch));
			}
		} catch (SomethingWentWrongException | NoRecordFoundException e) {
			System.out.println(ConsoleColors.RED + "		Product Not Available" + ConsoleColors.RESET);
		}
	...
	static void viewProduct(Scanner sc) {
		...
		try {
			...
			if (x.equals("1")) {
				System.out.print("	Enter Category Id : ");
				String ch = sc.next();
				dao.viewProductByCategory(Integer.parseInt(ch));
			}
		} catch (SomethingWentWrongException | NoRecordFoundException e) {
			System.out.println(ConsoleColors.RED + "		Product Not Available" + ConsoleColors.RESET);
		}
